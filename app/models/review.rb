class Review < ApplicationRecord
  ...
  def self.get_host_profile_reviews_length(host_profile_id)
    if Rails.cache.fetch("reviews_length_of_host_profile_with_id#{host_profile_id}").nil?
      Rails.cache.fetch("reviews_length_of_host_profile_with_id#{host_profile_id}") { Review.where(host_profile_id: host_profile_id).length }
