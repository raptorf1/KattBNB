    ...
    accepted_bookings.each do |booking|
      ...
      when 10
        ReviewsMailer
          ...
          .notify_user_pending_review_10_days(host[0], user[0], booking)
        notify_10.push(booking)
      when 3
        ReviewsMailer
          .delay(queue: 'reviews_email_notifications')
          .notify_user_pending_review_3_days(host[0], user[0], booking)
        notify_3.push(booking)
      when 1
        ReviewsMailer
          .delay(queue: 'reviews_email_notifications')
          .notify_user_pending_review_1_day(host[0], user[0], booking)
