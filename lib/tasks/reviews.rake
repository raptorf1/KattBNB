  ...
  task notify_pending_review: :environment do
    ...
    all_accepted_bookings.each do |booking|
      ...
      if !host.nil? && !user.nil?
        ...
        when 3
          ReviewsMailer.delay(queue: "reviews_email_notifications").notify_user_pending_review_3_days(
            host,
            user,
            booking
          )
        ...
        when 1
          ReviewsMailer.delay(queue: "reviews_email_notifications").notify_user_pending_review_1_day(
            host,
            user,
            booking
          )
